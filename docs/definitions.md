...
| scope | ENUM('hashes','wrappers','signatures') | NO |  | What is being migrated (hashes, wrappers, signatures). |
| selection | JSON | YES |  | JSON selector describing the affected dataset. |
| scheduled_at | DATETIME(6) | YES |  | Scheduled start time. |
| started_at | DATETIME(6) | YES |  | When the job started. |
| status | ENUM('pending','running','done','failed','cancelled') | NO | pending | Execution status. (enum: pending, running, done, failed, cancelled) |
