...
namespace BlackCat\Database\Packages\PqMigrationJobs\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\PqMigrationJobs\Dto\PqMigrationJobDto;
use BlackCat\Database\Packages\PqMigrationJobs\Mapper\PqMigrationJobDtoMapper;
 ...
 */
final class PqMigrationJobsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private PqMigrationJobRepository $pqMigrationJobRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'pq_migration_jobs' => $this->pqMigrationJobRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\PqMigrationJobs\ModuleException("Record {$idStr} not found for locking.");
            }
