...
namespace BlackCat\Database\Packages\PqMigrationJobs\Repository;
...
use BlackCat\Database\Packages\PqMigrationJobs\Criteria;
use BlackCat\Database\Packages\PqMigrationJobs\Dto\PqMigrationJobDto as Dto;
use BlackCat\Database\Packages\PqMigrationJobs\Mapper\PqMigrationJobDtoMapper as RowMapper;
...
final class PqMigrationJobRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\PqMigrationJobs\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\PqMigrationJobs\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PqMigrationJobs\Dto\PqMigrationJobDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\PqMigrationJobs\Dto\PqMigrationJobDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\PqMigrationJobs\Mapper\PqMigrationJobDtoMapper::fromRow($row);
    }
