...
namespace BlackCat\Database\Packages\PqMigrationJobs;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\PqMigrationJobs\Criteria;
use BlackCat\Database\Packages\PqMigrationJobs\Repository\PqMigrationJobRepositoryInterface;
use BlackCat\Database\Packages\PqMigrationJobs\Repository\PqMigrationJobRepository as GeneratedRepository;
...
{
  private PqMigrationJobRepositoryInterface $repo;
  ...
  public function __construct(private Database $db)
  {
    ...
    public function _setRepositoryForTests(RepoContract $repo): void
    {
        ...
        if (!$repo instanceof PqMigrationJobRepositoryInterface) {
            // Allow any repo that conforms to the interface even if not generated
            throw new \InvalidArgumentException('Expected PqMigrationJobRepositoryInterface');
        }
        $this->repo = $repo;
    }
    ...
    public function getByUnique(array $keys, bool $asDto = false): array|\BlackCat\Database\Packages\PqMigrationJobs\Dto\PqMigrationJobDto|null { return $this->repo->getByUnique($keys, $asDto); }
    public function getById(int|string|array $id, bool $asDto = false): array|\BlackCat\Database\Packages\PqMigrationJobs\Dto\PqMigrationJobDto|null { return $this->repo->getById($id, $asDto); }
